...
- **Java SE Development Kit 17**: We will be utilizing Java 17 as our primary language for the duration of the program. This software development kit will provide the compiler and runtime environment needed to quickly and efficiently develop Java applications. NOTE: While we will only cover topics and features of Java up to Java 8, Java 17 is being installed to better intigrate with some other tools we will be using. Java 17 is backwards compatable with all features of Java 8 we will be discussing. 
...
- **Docker Desktop**: Docker is a containerization plateform used to ensure consistent virtual enviorments for applications. We will be using it in a variety of ways throughout training. The easiest way to install all the required Docker compoents neccessary is to install Docker Desktop which comes with a convienent GUI to interact with Docker. 
...
**NOTE**: This guide primarily is written for those on a Windows Machine. Training equipment requirements specify Windows machines. While it is possible to complete training with another Operating System (OS) you will be responsable for recognizing and addressing any differences or issues that arise during training from using a non-supported OS. 
...
Now that you've installed Scoop and added some additional buckets for downloading software, open a new terminal and run the following command in order to install Java:
    ...
    scoop install ojdkbuild17-full
...
Docker Desktop cannot be installed via scoop. As such you will need to follow the manual process laid out on the [Docker Desktop](https://docs.docker.com/desktop/install/windows-install/) site. The WSL2 2 backend is recommended. 
...
1. Navigate in your web browser to [Oracle's JDK 17](https://www.oracle.com/java/technologies/downloads/#java17) download page. NOTE: If you are experiencing errors when trying to load the webpage try another browser. If you still are having trouble, try again later. 
...
5. Return to [Oracle's JDK 17](https://www.oracle.com/java/technologies/downloads/#java17) website. Make sure you click the Windows tab to get the Windows installer and click to download. 

![](./images/JDK-Download.jpg)

6. A popup will ask you to review the license before continuing. Do so and click the check box and then the download button. NOTE: Some of the following images will show a Java 8 installation/version but the process will be the same for Java 17.  
...
12. The installer will continue and ask you for the location to install Java. Generally the default location is recommended. IMPORTANT: Make note of the file path for the installation directory! You WILL need this later. Click "Next."

![](./images/JDK-Wizard2.jpg)

13. The install will begin extracting files.
...
Docker Desktop has it's own guide for installing. It be found at the [Docker Desktop](https://docs.docker.com/desktop/install/windows-install/) site. The WSL2 2 backend is recommended.
